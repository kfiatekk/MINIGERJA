...
public class QuizManager : MonoBehaviour
{
    ...
    public int remainingAttempts = 3; // Dodano zmienn¹ do œledzenia prób

    public void Start()
    {
        if (isQuizCompleted)
        {
            Debug.Log("Quiz zosta³ ju¿ ukoñczony z sukcesem. Nie mo¿esz go ponownie rozwi¹zaæ.");
            return;
        }

        if (!canRetryQuiz)
        {
            Debug.Log("Ostatni wynik by³ poni¿ej progu. Mo¿esz spróbowaæ ponownie.");
        }
        ...
        questions = new List<Question>
        {
            new Question
{
    questionText = "Do czego sluzy zmienna?",
    options = new List<string> { "Petla", "Dane", "Warunek" },
    correctOption = "B"
}
,
            new Question
    ...
    void DisplayQuestion()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                remainingAttempts--; // Zmniejsz liczbê prób
                canRetryQuiz = remainingAttempts > 0; // Blokada, jeœli brak prób
            }
